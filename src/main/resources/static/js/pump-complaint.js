...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!canSubmitComplaint(loggedInUser.role)) {
        alert("Only vehicle owner, building manager, or hospital authority can submit pump complaints.");
...
function canSubmitComplaint(role) {
...
function setupPageTitle() {
    if (loggedInUser.role === "VEHICLE_OWNER") {
        setText("complaintPageTitle", "Submit Pump Complaint");
        setText("complaintPageSubtitle", "Submit a complaint against a pump for vehicle fuel service issues.");
        setText("complaintRoleBadge", "VEHICLE OWNER");
        return;
    }

    if (loggedInUser.role === "BUILDING_MANAGER") {
        setText("complaintPageTitle", "Building Pump Complaint");
        setText("complaintPageSubtitle", "Submit a complaint against a pump for building generator diesel collection issues.");
        setText("complaintRoleBadge", "BUILDING MANAGER");
        return;
    }

    if (loggedInUser.role === "HOSPITAL_AUTHORITY") {
        setText("complaintPageTitle", "Hospital Pump Complaint");
        setText("complaintPageSubtitle", "Submit a complaint against a pump for hospital generator diesel or critical support issues.");
        setText("complaintRoleBadge", "HOSPITAL AUTHORITY");
    }
}
...
async function submitComplaint() {
    const data = {
        ...
        complaintDescription: buildDescriptionWithRoleContext(),
...
function buildDescriptionWithRoleContext() {
    const description = getValue("complaintDescription");

    if (loggedInUser.role === "BUILDING_MANAGER") {
        return "Complainant Role: BUILDING_MANAGER\n"
            + "Building: " + valueOrDash(loggedInUser.buildingName) + "\n"
            + "Building Thana: " + valueOrDash(loggedInUser.buildingUnderThana || loggedInUser.thanaOrUpazila) + "\n\n"
            + description;
    }

    if (loggedInUser.role === "HOSPITAL_AUTHORITY") {
        return "Complainant Role: HOSPITAL_AUTHORITY\n"
            + "Hospital: " + valueOrDash(loggedInUser.hospitalName) + "\n"
            + "Hospital Thana: " + valueOrDash(loggedInUser.hospitalUnderThana || loggedInUser.thanaOrUpazila) + "\n\n"
            + description;
    }

    return "Complainant Role: VEHICLE_OWNER\n\n" + description;
}
...
async function loadMyComplaints() {
    ...
    try {
        const response = await fetch("http://localhost:8081/api/pump-complaints/user/" + userId + "?time=" + Date.now());
        const result = await response.json();
